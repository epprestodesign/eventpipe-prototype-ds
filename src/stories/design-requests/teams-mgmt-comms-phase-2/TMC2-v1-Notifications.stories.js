/** Teams Mgmt Comms Phase 2 / V1 · Notifications Preferences — full App Shell experience.
 *  Forked from the DES-207 story so this track can evolve independently; the
 *  whole fork (stories, DS components, app reference code) lives under this
 *  directory and shares nothing with DES-207 but the App Shell and logo asset.
 *
 *  Template copy lives in ./tmc2-content.json. Editing is done through
 *  Storybook's native Controls panel (a Header + Subtext control per template).
 *
 *  NOTE: the "Publish content" toolbar addon (.storybook/manager.js) is
 *  deliberately NOT wired up here. It detects its story by the `s{N}i{M}_title`
 *  arg keys and commits to des207-content.json — so this fork prefixes its arg
 *  keys with `tmc2` to stay out of the way. Controls still edit copy live, but
 *  changes are session-only; edit tmc2-content.json to persist. To give this
 *  track its own publish target, teach manager.js to pick the content file
 *  based on which arg-key prefix is present. */
import { ref, watchEffect } from 'vue'
import { useQuasar } from 'quasar'
import { page } from '../../pages/_shell'
import { travelocHeader, companySettingsSections } from './_tmc2'
...
export default {
  title: 'Design Requests/Teams Mgmt Comms Phase 2/V1 · Notifications Preferences',
  tags: ['autodocs'],
