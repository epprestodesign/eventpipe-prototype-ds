<script setup>
// DsRichTextEditor — the Email Template "Content" editor (Teams Mgmt Comms Phase 2). A WYSIWYG on
...
// tokens insert as plain `{{token_key}}` (per DES-430) — via the toolbar button
// OR by typing `{{`, which pops the same menu inline at the caret.
...
const props = defineProps({
  ...
  // Toolbar layout: 'email' (default — Teams Mgmt Comms Phase 2 template editor) or 'description'
...
function insertToken(token) {
  restoreSel()
  editorRef.value?.runCmd('insertText', `{{${token}}}`)
  saveSel()
}
...
<template>
  <DsField :label="label" :required="required">
    <div class="dsrte">
      ...
      <!-- 'email' layout (default): Teams Mgmt Comms Phase 2 template editor toolbar -->
